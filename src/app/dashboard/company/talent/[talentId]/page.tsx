...
type CompanyTalentProfileResponse = {
  profile?: Profile;
...
export default function CompanyTalentProfilePage() {
  const { talentId } = useParams<{ talentId: string }>();

  const [data, setData] = useState<CompanyTalentProfileResponse | null>(null);
  ...
  const load = async () => {
    try {
      ...
      const res = await fetch(`/api/company/talent/${talentId}/profile`, {
        cache: "no-store",
      });
      ...
      const parsed = text ? (JSON.parse(text) as CompanyTalentProfileResponse) : {};
      setData(parsed ?? {});
    } catch (e: any) {
      setData(null);
  ...
  const computed = useMemo(() => {
    const count = data?.review_count ?? reviews.length;
    const avg =
      data?.average_rating ??
      (reviews.length
        ? reviews.reduce((s, r) => s + clamp(Number(r.rating) || 0, 0, 5), 0) / reviews.length
        : 0);
  ...
  const fullName =
    `${profile.first_name ?? ""} ${profile.last_name ?? ""}`.trim() || "Professional";

  const headlineBits = [
    profile.profession || null,
    profile.engineering_discipline || null,
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-white">Professional profile</h1>
          <p className="mt-1 text-sm text-neutral-400">
            Review this professional before requesting to book or inviting them to apply.
          </p>
        </div>
        ...
        <>
              ...
              {profile.cv_url ? (
                <div className="mt-4">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex rounded-xl border border-purple-500/40 bg-purple-950/30 px-4 py-2 text-xs font-medium text-purple-200 transition hover:bg-purple-900/40"
                  >
                    View CV
                  </a>
                </div>
                      ...
                      <li key={qualification.id} className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 text-xs text-neutral-500">
                            {[qualification.issuer, qualification.credential_ref]
                              .filter(Boolean)
          ...
          <div className="rounded-3xl border border-neutral-800/80 bg-neutral-950/60 shadow-[0_0_40px_rgba(0,0,0,0.55)] overflow-hidden">
            <div className="border-b border-neutral-800/80 px-6 py-4 flex items-center justify-between">
              ...
              <div className="text-xs text-neutral-500">Showing visible verified reviews</div>
            </div>
                        ...
                        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              ...
                              <div className="text-xs text-neutral-400">
                                {fmtDate(r.verified_at || r.created_at)}
                              </div>
                            </div>
                          ...
                          <div className="md:text-right text-xs text-neutral-400">
                            <div className="text-neutral-200 font-medium">
                              {r.reviewer_name || "Verified reviewer"}
                            </div>
